...
st.caption("Click a city on the map to select it, then configure parameters and run the optimizer.")
...
with map_col:
    ...
    for city_name, (lat, lon) in CITIES.items():
        is_selected = (city_name == selected)

        # Outer ring for selected city
        if is_selected:
            folium.CircleMarker(
                location=[lat, lon],
                radius=18,
        ...
        folium.Marker(
            ...
            icon=folium.DivIcon(
                html=f"""
                <div style="
                    background: {'#1d4ed8' if is_selected else '#ffffff'};
                    color: {'#ffffff' if is_selected else '#1e293b'};
                    border: 2px solid {'#1d4ed8' if is_selected else '#94a3b8'};
                    border-radius: 50%;
                    width: 30px;
                    height: 30px;
                    display: flex;
    ...
    # Detect clicked city from popup
    clicked_popup = (map_data or {}).get("last_object_clicked_popup")
    if clicked_popup and clicked_popup in CITIES:
        st.session_state.selected_city = clicked_popup
        selected = clicked_popup
        st.rerun()

    # City name display below map
    if selected:
...
# ══════════════════════════════════════════
with ctrl_col:
    st.markdown('<div class="section-label">Building parameters</div>',
                unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-label" style="margin-top:12px;">Inflation rates</div>',
                unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    elec_inf = c1.number_input("Electricity", value=0.01, min_value=0.0,
                               max_value=0.5, step=0.005, format="%.3f")
    fuel_inf = c2.number_input("Fuel", value=0.05, min_value=0.0,
                               max_value=0.5, step=0.005, format="%.3f")

    st.markdown('<div class="section-label" style="margin-top:12px;">Objective weights</div>',
                unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-label" style="margin-top:12px;">Sampling</div>',
                unsafe_allow_html=True)
...
if run and selected:
    ...
    # ── Results ──────────────────────────────────
    st.markdown("---")
    st.success(f"✅ Best retrofit found for **{city}** under **{ssp}** "
    ...
    st.caption(f"⏱️ Estimated payback: **{payback if payback > 0 else '< 1'} years** "
               f"(net cost ${net_cost:,.0f} ÷ ~${annual:,.0f}/yr savings)")
    ...
    with res1:
        st.subheader("Best retrofit parameters")
        param_df = pd.DataFrame([
            {"Parameter": PARAM_LABELS[k],
             "Value": f"${best[k]:,.0f}" if k in ("Loan","Rebate") else f"{best[k]:.3f}",
             "Range": f"{RANGES[k][0]} – {RANGES[k][1]}"}
            for k in keys
        ])
    ...
    with res2:
        ...
        fig_hist.add_vline(x=best["Score"], line_dash="dash", line_color="#D85A30",
                           annotation_text="Best", annotation_font_size=10)
        fig_hist.update_layout(margin=dict(t=35,b=10,l=10,r=10), height=190,
        ...
        fig_radar.update_layout(
            polar=dict(radialaxis=dict(range=[0,1], showticklabels=False,
                                       gridcolor="#f3f4f6")),
            margin=dict(t=20,b=20,l=40,r=40), height=190,
            paper_bgcolor="white",
    ...
    with st.expander("📊 Sensitivity — which variables drive the score?"):
        ...
        fig_sens = go.Figure(go.Bar(
            ...
            y=[PARAM_LABELS[k] for k in corr_sorted],
            orientation="h",
            marker_color="#3266ad", marker_line_width=0,
        ))
        fig_sens.update_layout(margin=dict(t=10,b=10,l=10,r=10), height=420,
    ...
    # ── Pareto ────────────────────────────────────
    with st.expander("🔵 Pareto frontier — owner savings vs GHG"):
        sample_plot = df.sample(min(500, len(df)), random_state=1)
        ...
        fig_par.add_scatter(
            ...
            marker=dict(size=16, color="#D85A30", symbol="star"),
            name="Best",
        )
        fig_par.update_layout(margin=dict(t=10,b=10,l=10,r=10), height=350,
                              plot_bgcolor="white", paper_bgcolor="white",
    ...
    st.download_button("⬇️ Download top 50 results (CSV)", data=csv,
                       file_name=f"retrofit_{city}_{ssp}.csv",
                       mime="text/csv")